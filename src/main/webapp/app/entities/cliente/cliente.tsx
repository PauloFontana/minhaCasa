    ...
    return (
      <div>
        <h2 id="cliente-heading">
          <Translate contentKey="minhaCasaApp.cliente.home.title">Clientes</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="minhaCasaApp.cliente.home.createLabel">Create new Cliente</Translate>
          </Link>
        </h2>
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {cliente.clienteProprietario ? (
                      <Link to={`cliente-proprietario/${cliente.clienteProprietario.id}`}>{cliente.clienteProprietario.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {cliente.clienteComprador ? (
                      <Link to={`cliente-comprador/${cliente.clienteComprador.id}`}>{cliente.clienteComprador.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
