import React from 'react';
import { Switch } from 'react-router-dom';

// tslint:disable-next-line:no-unused-variable
import ErrorBoundaryRoute from 'app/shared/error/error-boundary-route';

import Cliente from './cliente';
import ClienteProprietario from './cliente-proprietario';
import ClienteComprador from './cliente-comprador';
import Imovel from './imovel';
/* jhipster-needle-add-route-import - JHipster will add routes here */

const Routes = ({ match }) => (
  <div>
    <Switch>
      {/* prettier-ignore */}
      <ErrorBoundaryRoute path={`${match.url}/cliente`} component={Cliente} />
      <ErrorBoundaryRoute path={`${match.url}/cliente-proprietario`} component={ClienteProprietario} />
      <ErrorBoundaryRoute path={`${match.url}/cliente-comprador`} component={ClienteComprador} />
      <ErrorBoundaryRoute path={`${match.url}/imovel`} component={Imovel} />
      {/* jhipster-needle-add-route-path - JHipster will routes here */}
    </Switch>
  </div>
);

export default Routes;
